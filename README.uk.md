...
<p align="center">
  <img src="https://img.shields.io/badge/release-v1.2.4.3--beta-1f6feb" alt="Випуск v1.2.4.3-beta">
...
<p align="center">
  <a href="https://github.com/mykola-zhukovets/balachky/releases/download/v1.2.4.3-beta/BalachkySetup-1.2.4.3-beta-2554A930.exe"><img src="https://img.shields.io/badge/Завантажити_для_Windows_(.exe)-2ea043?style=for-the-badge" alt="Завантажити Балачки v1.2.4.3 beta для Windows" height="44"></a>
  <br>
  <a href="https://github.com/mykola-zhukovets/balachky/releases/tag/v1.2.4.3-beta">Відомості про випуск</a>
  <br>
...
**100 мов розпізнавання у v1.2.4.3-beta · Автоматичне визначення мови · Інтерфейс українською та англійською**
...
Зі сторінки наради розшифровку можна експортувати й у субтитри для відеоплеєра чи сайту (SRT, VTT), за бажанням із позначками, хто говорить (Я / Співрозмовники).
...
Історія диктувань зберігається на Вашому комп’ютері; голосові з інших пристроїв (отримані через віддалене диктування) показані в окремій секції. Шукайте попередні диктування, переглядайте статистику, копіюйте або виправляйте текст, видаляйте записи й експортуйте потрібні результати.
...
Записуйте весь монітор або одне вікно в окремому режимі. У v1.2.4.3-beta пункт “Ділянка” ще записує перший монітор, а не довільно обраний фрагмент.
...
- **Озвучення тексту:** інтерфейс є, але рушій озвучення не входить до публічного інсталятора v1.2.4.3-beta.
- **Моделі розпізнавання:** до типової моделі додалися ще дві. `Parakeet` (NVIDIA, через sherpa-onnx) охоплює українську та ще 24 європейські мови й працює на процесорі — відеокарта не потрібна; на власних 20 записах українською ця модель помилялася рідше (4,55% слів), ніж типова модель на відеокарті (5,84%). Модель `large-v2` — альтернатива найточнішій моделі, `large-v3`, для записів, де та домислює слова: на тому самому наборі `large-v3` дала 1,95% помилок, а `large-v2` — 3,25%. Пакет Parakeet (~670 МБ) завантажується за Вашою згодою з перевіркою кожного файла; ліцензію (CC-BY-4.0) показано перед завантаженням.
- **Віддалене диктування через Telegram:** підключення власного бота дозволяє надсилати голосові повідомлення з телефона на ПК для локального розпізнавання (детальніше: [docs/TELEGRAM.md](docs/TELEGRAM.md)).
...
| Перевірка з’єднання в майстрі налаштування | v1.2.4.3-beta створює TCP-з’єднання з `1.1.1.1:53`, коли відкривається майстер налаштування; перевірка не надсилає голосу чи розшифровок |
...
| Віддалене диктування через Telegram | Лише поки підключено бота: Балачки обмінюються даними з `api.telegram.org`, щоб приймати голосові й надсилати розшифровки |
...
Балачки працюють у 64-розрядній Windows 10 або 11. Інсталятор займає 161,8 МіБ (169,6 МБ; 169 609 456 байтів). Для моделей розпізнавання й додаткових компонентів потрібне окреме місце на диску; загальний обсяг залежить від Вашого вибору.

Завантажте `BalachkySetup-1.2.4.3-beta-2554A930.exe` з [офіційної сторінки випуску v1.2.4.3-beta](https://github.com/mykola-zhukovets/balachky/releases/tag/v1.2.4.3-beta) і запустіть інсталятор. Для завантаження моделі потрібен інтернет, якщо Ви не імпортуєте файли з офлайн-пакета.
...
Перевірити цей файл на VirusTotal за його SHA-256: [сторінка файла](https://www.virustotal.com/gui/file/2554a930b2bb8da1ea27790907e0656eb8d7cd60859e3193b32926d3fa74371e/detection). Якщо звіту ще немає, файл можна надіслати на перевірку самостійно. Результат сканування є додатковою перевіркою, а не заміною звіряння SHA-256.
