...
# "C:\Users\<логін>" на нейтральну позначку, лишаючи решту шляху для
# діагностики. Живе тут (не в fronts/desktop), бо цією ж функцією користується
# і whisper_core (whisper_core/meeting/session.py, whisper_core/recordings.py),
# і GUI-шар (fronts/desktop/crash.py re-export) — utils-шар не залежить від UI.
#
# Корінь перед "Users" — або звичайний диск ("C:"), або мережевий шлях
# ("\\server" / "\\server\share"): військові й медичні профілі часто на
# мережевому диску. Обидва варіанти, а не лише диск — інакше
# "\\server\Users\ivan\..." проходить журнал сирим (знахідка рецензента 31.07).
# Увесь вираз регістронезалежний (re.IGNORECASE) — "C:\USERS\ivan" ловиться
# так само, як "c:\users\ivan"; раніше гнучкою була лише перша літера диска.
# Навмисно НЕ прив'язуємось до кореня шляху: атаки рецензентів 31.07 показали,
# що будь-який перелік коренів (диск, \\server, \\server\share, \\?\UNC\...)
# рано чи пізно обходиться ще однією формою. Ловимо сам сегмент "\Users\<хтось>"
# де завгодно. Ціна — теоретичне зайве знеособлення рядків на кшталт
# "/users/..." у веб-адресах; для журналу приватність важить більше за це.
# Роздільник — 1-2 бекслеші АБО слеш: OSError.__str__ (і будь-який traceback)
# показує шлях через repr() з подвоєними "\\", і одинарний клас символів
# пропускав "C:\\Users\\ivan" сирим (атака рецензента №4, 31.07).
...
def anonymize_path(value) -> str:
    """Замінити ім'я облікового запису Windows у шляху на "<користувач>".

    Приймає Path/str/None (None чи помилка перетворення → порожній рядок).
    Годиться і для одиничного шляху, і для багаторядкового тексту (регулярний
    вираз шукає збіги скрізь) — тому підходить і для окремих аргументів
    logging.*, і для санітизації цілого файлу логу перед пакуванням у звіт.
    """
    if value is None:
        return ""
    try:
        ...
        return ""
    return _USER_SEGMENT_RE.sub(r"\1<користувач>", text)
