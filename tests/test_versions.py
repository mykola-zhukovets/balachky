...
class VersionConstantsTests(unittest.TestCase):
    def test_release_concepts_have_distinct_canonical_values(self):
        self.assertEqual(
            getattr(whisper_core, "DISPLAY_VERSION", None), "1.2.4-beta")
        self.assertEqual(
            getattr(whisper_core, "PEP440_VERSION", None), "1.2.4b0")
        self.assertEqual(
            getattr(whisper_core, "WINDOWS_FILE_VERSION", None), (1, 2, 4, 0))
        self.assertEqual(
...
class UpdateVersionComparisonTests(unittest.TestCase):
    def test_stable_release_is_newer_than_same_base_beta(self):
        ...
        self.assertTrue(updates.is_newer("1.2.4", current))
...
class WindowsVersionBuildTests(unittest.TestCase):
    def test_spec_uses_numeric_file_version_when_display_has_suffix(self):
        ...
        version_values = {
            "DISPLAY_VERSION": "1.2.4-beta",
            "PEP440_VERSION": "1.2.4b0",
            "WINDOWS_FILE_VERSION": (1, 2, 4, 0),
            "RELEASE_CHANNEL": "beta",
        }
        namespace = {"SPECPATH": str(ROOT)}
        with patch.object(
                Path, "read_text", return_value='__version__ = "1.2.4-beta"\n'), \
        ...
        self.assertEqual(file_version, (1, 2, 4, 0))
