...
class ProcessingSlider(QWidget):
    ...
    def __init__(self, surface: str, *, caption: str = "", parent=None):
        ...
        # Слайдер стилізує chip_popover.make_slider («як у Claude»): ледь помітна
...
class ProcessingChip(QWidget):
    """Т47 — компактний чіп рівня обробки біля кнопки запису (ідіома Effort-чіпа
    Claude Code). Чіп показує поточний рівень («Обробка: Дослівно»); клік
